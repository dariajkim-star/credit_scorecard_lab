...
Lending Club의 `grade`/`sub_grade`/`int_rate`는 **LC 자체 심사 모형의 출력(및 그 결과로 정해진 계약 조건)**입니다. 배제 근거는 두 가지입니다: ①**타사 스코어의 대리변수** — 이를 피처로 쓰면 내 모형의 판별력이 아니라 남의 모형을 복제하는 것이 됩니다. ②**자사 신규 심사 시점에 부재** — 우리가 이 스코어카드로 직접 심사하는 상황에는 LC의 등급·금리가 존재하지 않습니다. → **보수적으로 배제**.

*(참고: §7.2의 손익 시뮬레이션은 실현 상환액(`total_pymnt`, 금리가 반영된 결과)을 사용하지만, 이는 **피처가 아니라 성과(outcome) 데이터**입니다 — 예측 입력으로 쓰는 것과 사후 평가에 쓰는 것은 다른 문제이며, §7.2에서 재론합니다.)*
...
- 점수 분포 496~601.
- **스케일 앵커의 한계**: "600점 = 50:1 odds" 앵커는 관행값이지 이 포트폴리오에 보정된 값이 아닙니다 — 실제 OOT 부도율 14.89%(전체 odds ≈ 5.7:1) 기준으로는 600점이 분포 최상단에 위치해 앵커의 관행적 의미와 어긋납니다. 따라서 점수는 **절대 odds 해석이 아니라 서열(랭킹) 도구**로 사용하며, cutoff·등급 등 모든 후속 산출이 서열 기반입니다.
...
## 5. 성능 평가 (train/valid/OOT 3분할) — 목표 미달과 그 원인
...
2. **grade·int_rate 의도적 배제(§2.5)**: 이들을 넣으면 AUC는 즉시 크게 오르지만, **타사 심사 결과의 대리변수이며 자사 신규 심사 시점에 존재하지 않는 값**입니다. 성능 숫자를 위해 방법론을 포기하지 않았습니다.
3. **train/valid/OOT 간 성능 차가 작음**(0.6468/0.6406/0.6430) — **과적합은 없습니다**. 다만 과적합 부재가 곧 "정보 상한 도달"의 증거는 아닙니다(과소적합과 구별되지 않음) — 7변수는 우리가 좁힌 선택이고, 배제 전 17변수 벤치마크를 별도로 산출하지 않은 것은 이 문서의 한계입니다. 현재 근거로 말할 수 있는 것은 "이 변수셋·이 제약 하에서는 추가 개선 여지가 제한적"까지입니다.
...
**해석 주의**: 빈티지 부도율이 12.70% → 14.89%로 뚜렷이 이동했는데 점수 PSI가 0.005 수준이라는 것은, 점수 분포가 안정적이라는 뜻인 동시에 **그 위험 이동을 점수가 담아내지 못한다**는 뜻이기도 합니다(낮은 KS와 정합적). 즉 이 PSI는 "운영 안정성" 지표로는 합격이지만 "환경 변화 민감도" 관점에서는 둔감함의 신호일 수 있어, 양면을 함께 읽어야 합니다.
...
- **챔피언 → 챌린저 교체 시**: `swap_out`(챔피언 승인·챌린저 거절) **15,115명 부도율 14.95%**, `swap_in`(반대 방향) **5,049명 부도율 13.55%**, `stable_approved` **8.81%** — 걸러내는 집단이 새로 받는 집단보다 위험하다는 **방향성을 시사**합니다. 다만 두 모형의 판별력 차이 자체가 작아(OOT AUC 0.6430 vs 0.6452) 교체의 실질 편익은 크지 않으며, swap 집단의 부도율이 안정 승인군보다 높은 것은 경계 집단의 일반적 특성이기도 합니다 — "교체 근거 확보"가 아니라 "교체 시 영향 집단의 정량화"가 이 분석의 기여입니다.

### 7.2 손익 기반 cutoff (컨설턴트 킥①) — 발견이자 진단 신호

리스크(부도율)가 아닌 **실현 손익**(`total_pymnt + recoveries − loan_amnt`)으로 cutoff을 평가한 결과:

| | 현재 cutoff | 손익 최적 cutoff | 차이 |
|---|---:|---:|---|
| 챔피언 | 546.01 | **494.43** | 승인율 **+52.29pp**, 연간 기대손익 **+$132M** |
| 챌린저 | 547.60 | **507.10** | — |

**결과를 정확히 읽으면**: 챔피언의 손익 "최적점" 494.43은 **점수 분포 최소값(496)보다 낮은 탐색 구간 하단 경계값**입니다 — 내부 최적점이 아니라 "이 표본에서는 사실상 전원 승인이 손익 최대"라는 뜻입니다. 이는 두 가지를 말해줍니다:

1. **accepted-only 데이터의 구조적 결과**: LC가 이미 저품질 신청을 거절한 뒤의 잔존 모집단이라, 최하위 점수 구간조차 이자수익이 부도손실을 상회합니다. 진짜 신청 모집단이라면 곡선 하단이 꺾일 것이고, 이 "최적점"은 §10.1의 reject inference 편향이 가장 직접적으로 드러나는 자리입니다.
2. **그럼에도 유효한 함의**: 승인 모집단 내부에서는 리스크 기준 cutoff(546.01)이 손익 관점에서 과도하게 보수적이라는 방향성 자체는 성립합니다 — 단, 정확한 최적점은 이 데이터로 결정할 수 없습니다.

*(가정: OOT 표본 = 1년 승인 볼륨, avg_loan_amnt=$12,000 스케일링, **통화는 데이터 원 통화(USD) 그대로**. 현재 cutoff 546.01은 Story 2.1 리포트의 데모 관행값이지 실제 운영 정책이 아니며, +52.29pp는 이 기준선에 상대적인 수치입니다. 손익 시뮬레이션이지 재무 데이터가 아닙니다.)*
...
|---|---:|---:|---:|---|
| DTI > 40 | **34** | 23.5% | 1.58배 | **91%를 모형이 이미 거절**(중복) — *n=34 소표본, 아래 주의* |
| 조회 ≥ 3 | 11,686 | 24.4% | 1.64배 | **85%를 모형이 이미 거절**(중복) |
| 연체 ≥ 1 | 58,974 | 16.0% | **1.07배** | 판별력 낮음, 기회손실 **$94.4M** |

**시사점**: 스코어카드가 있으면 이 하드룰들은 중복이거나 비효율입니다. 특히 연체 룰은 모집단의 21%를 거절하면서 판별력이 거의 없어 우량 고객을 대량으로 잃습니다.

**증거 강도 주의**: DTI 룰의 배제는 **34건**뿐이라(283,026건 중) 부도율 23.5%의 표준오차가 ±7pp 수준이고 "91% 중복"도 31명 규모입니다 — 이 룰의 판정은 다른 두 룰(각 1.2만·5.9만 건)과 증거 강도가 다르며, 방향성 참고로만 읽어야 합니다. (LC accepted 데이터엔 고DTI 신청이 이미 걸러져 있어 표본이 작은 것 자체가 §10.1의 선택 편향 흔적입니다.)
...
점수 산출 로직(WOE 룩업 → 선형결합 → PDO 스케일)을 SAS로 이식. 아티팩트에서 **자동생성**해 손 복사 오차를 원천 차단했고, **미러**(SAS와 동일한 룩업·공식을 라이브러리 없이 순수 산술로 재구현한 Python 검증기)로 대조한 결과 **12건 최대 오차 4.74e-07점**(기준 0.5점). "Python으로 개발, SAS로 이식 검증"이 가능함을 증빙합니다.
...
| 손익 최적 cutoff | **승인 모집단 내에서는 CX(승인율)와 수익이 같은 방향** — 승인율 +52.29pp가 손익도 +$132M (§7.2). 단 이 결론은 accepted-only 편향(§7.2·§10.1) 조건부이며, 신청 모집단 전체로 일반화할 수 없음 — CX 개선 논거를 수익 언어로 번역하는 **방법의 예시**로 읽을 것 |
| 룰 효율성 진단 | **불필요한 거절 제거** — 판별력 없는 연체룰이 우량 고객 배제, 기회손실 $94.4M = 나쁜 CX의 비용화 |
...
| **Parcelling** | 거절 건에 모형 점수를 매기고, 점수 구간별 부도율(에 배수 적용)에 비례해 good/bad를 **확률적으로 배정(random assignment)** | 같은 점수면 승인·거절 집단의 위험이 (배수 조정 후) 같다 | 배수가 **자의적** — 결국 가정을 데이터로 검증할 수 없음 |
