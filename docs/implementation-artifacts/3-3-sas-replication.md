...
- [x] Task 1: 챔피언 점수 산출 로직 추출 (AC: #1, 전제)
  - [x] **이식 대상 확정(실측 확인)**: `champion.score_applicant` 경로 = 원시값 →(WOE 룩업)→ `logit = intercept + Σ(coef_i · woe_i)` →(PDO 스케일)→ `score = offset + factor·(−logit)`, `factor = PDO/ln(2)`, `offset = base_score − factor·ln(base_odds)`. 상수: PDO=20, base_score=600, base_odds=50(`champion.py`). intercept=−1.928423, 7개 계수 전부 음수(fico −0.7686 … inq −1.0981) — 아티팩트에서 실측.
  - [x] `sas/export_scorecard.py`(또는 유사) — 챔피언 아티팩트(`champion_model.joblib`)에서 **정확히** 추출: (a) 7변수 각각의 WOE 빈 테이블(수치 변수=구간 경계→WOE, 범주 변수 home_ownership/purpose=카테고리→WOE), (b) 로지스틱 계수·절편, (c) 상수. **Missing/Special 빈의 WOE는 binning_table의 실측값 사용**(서빙 `transform_woe`가 `metric_missing="empirical"`이므로 SAS도 동일 empirical WOE로 매핑 — 0 하드코딩 금지, 1.4 함정 계승).
  - [x] 추출 산출물을 SAS가 소비할 형태로: 권장 — 추출값을 그대로 `.sas`에 하드코딩 생성(외부 파일 의존 없이 OnDemand에 붙여넣기 가능). 부동소수 정밀도는 최소 6자리 유지(오차 <0.5 여유 확보).
- [x] Task 2: `scorecard_scoring.sas` 생성 (AC: #1)
  - [x] SAS DATA step: 각 변수의 원시값 → WOE (수치=`if lo <= x < hi then woe=...` 체인, 범주=`if var in (...) then woe=...`), 미해당/결측 → 해당 변수의 Missing WOE. **경계 방향은 Python 빈과 동일하게**(optbinning은 `[lo, hi)` 좌폐우개 — SAS도 동일 부등호). 그다음 `logit = intercept + sum(coef*woe)`, `score = offset + factor*(-logit)`.
  - [x] 결정론적·자기완결적: 외부 매크로·PROC 의존 최소화(순수 DATA step 권장), OnDemand에서 그대로 실행 가능하게. 상단 주석에 "이 파일은 champion 아티팩트에서 자동생성됨 + 재생성 방법" 명시.
  - [x] **자동생성**을 권장(손으로 WOE 표 옮기면 오타 리스크) — `export_scorecard.py`가 `.sas` 텍스트를 통째로 써내도록. 값이 아티팩트와 100% 일치 보장.
- [x] Task 3: Python 기준 점수 + SAS 로직 미러 (AC: #1)
  - [x] `sas/reference_applicants.csv` — OOT(2015)에서 10건+ 신청의 원시 입력 7필드 + `champion.score_applicant` 점수(Python 정답). 결측·범주 케이스 포함(경계·Missing 매핑까지 커버되게 다양하게 선정).
  - [x] **Python SAS-로직 미러**(`sas/sas_logic_mirror.py` 또는 export의 일부): sklearn/optbinning 없이 **순수 산술**로 SAS와 동일한 WOE 룩업+선형결합+PDO를 재구현. 이것이 `champion.score_applicant`와 10건 전부 <0.5(사실상 <1e-6) 일치함을 검증 → **SAS가 미러할 산술이 정확함을 라이브러리 독립적으로 증명**. (SAS 실행 없이도 이식 로직의 정확성을 이 세션에서 보증하는 핵심 장치.)
- [x] Task 4: 대조 리포트 (AC: #2)
  - [x] `docs/implementation-artifacts/sas-replication-report-3-3.md` — 이식 대상 로직·공식, WOE 룩업 방식, 상수, **대조 절차**(사용자가 OnDemand에서 `.sas` 실행 → 출력 점수 → `reference_applicants.csv`의 Python 점수와 차이 계산 → 전 건 <0.5 확인), Python 미러 검증 결과(이 세션에서 산출), 한계(SAS 최종 tie-out은 사용자 실행 의존). "Python 개발 / SAS 이식 검증" 역량 증빙 톤.
- [x] Task 5: pytest + 실증 (AC: 전체)
  - [x] `tests/test_sas_replication.py` — SAS 로직 미러가 `champion.score_applicant`와 10건+ 일치(<0.5, 실제 <1e-6), 경계값·범주·Missing 케이스 포함. export가 아티팩트 값과 일치(계수/절편/WOE). reference CSV 생성 재현성.
  - [x] `pytest -q` 전체 통과(기존 210 + 신규).
  - [x] 실데이터: 실제 champion 아티팩트에서 SAS 자동생성 + 10건 미러 대조 실행(이 세션). SAS 실제 실행은 사용자 몫으로 명확히 인계(리포트에 절차).
...
- `binning_table.build()`의 `Bin` 컬럼이 categorical 변수에선 numpy 배열이라 `== "Missing"` 벡터 비교가 ambiguous ValueError — 행 단위 `isinstance(str)` 비교로 교체.
- **미학습 카테고리의 실측 동작 발견**: `transform_woe`는 unseen 카테고리를 Missing이 아니라 **Special 빈 WOE(0.0)**로 매핑(사전 실측). 스토리 노트의 "미학습→Missing" 가정을 실측에 맞춰 수정 — 미러·SAS 모두 Special 관례 채택, 리포트에 기록.

### Completion Notes List

- **미러 대조 결과(AC #1의 세션 내 증명)**: OOT 12건(결측 revol_util 3건 포함)에서 순수 산술 미러 vs 라이브러리 경로(`transform_woe`+`decision_function`+`score_formula`) **최대 절대 오차 4.74e-07점** — FR17 기준(0.5점)의 백만분의 일 수준. 회귀 테스트로 <0.5와 <1e-6 이중 고정.
- **자기완결 `.sas`(140줄)**: 12건 reference가 datalines로 내장, 변수별 WOE if/else 체인(수치 5개=좌폐우개 경계, 범주 2개=동일-WOE 그룹 `in` 절), logit+PDO 공식, **diff·pass 컬럼까지 출력** — OnDemand에 붙여넣고 실행만 하면 tie-out 완료. 상단에 AUTOGENERATED+재생성 명령 주석.
- **아티팩트 단일 진실(AD-1)**: `extract_scorecard`가 joblib에서 계수/절편/WOE 전량 추출, 추출값=아티팩트 일치를 테스트로 고정. 손 복사 0.
- **결정 기록**: revol_util은 reference에 파싱된 float로 담아 SAS 문자열 처리 제거. 미학습 카테고리→Special WOE(실측 관례).
- **사용자 인계**: 리포트에 OnDemand 실행 절차 4단계 + 기대 결과(전건 pass) 명시. AC #3(비블로킹) 충족 — 다른 스토리와 독립.
- pytest **221 passed**(기존 215 + sas 6).

### File List

- `sas/__init__.py` (NEW)
- `sas/export_scorecard.py` (NEW — extract_scorecard/mirror_score/build_reference_applicants/generate_sas/write_outputs)
- `sas/scorecard_scoring.sas` (NEW — 자동생성, 140줄 자기완결)
- `sas/reference_applicants.csv` (NEW — 12건 원시입력+python_score)
- `tests/test_sas_replication.py` (NEW — 6 tests)
- `docs/implementation-artifacts/sas-replication-report-3-3.md` (NEW — 대조 절차·미러 결과·한계)

## Change Log

- 2026-07-16: Story 3.3 구현 — sas/export_scorecard.py(아티팩트→.sas 자동생성+순수 미러). 미러 대조 12건 최대 오차 4.74e-07(기준 0.5). 실측 발견: unseen 카테고리는 Special 빈 매핑(스토리 가정 수정). 자기완결 .sas(datalines 내장+diff 출력)로 사용자 OnDemand tie-out만 남김. pytest 221 passed(+6). Status → review.
