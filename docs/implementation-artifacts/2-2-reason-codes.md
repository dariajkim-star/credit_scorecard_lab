...
변수 i의 `points_lost`(양수 = 손실) = `factor * coef_i * (safest_woe_i - applicant_woe_i)`.
...
- `scorecard/reasons.py` (MODIFIED — 스텁 → 구현: ReasonCode/ChampionReasonCode/ChallengerReasonCode, `_normalize_raw_applicant`, `_safest_woe`, `champion_reason_codes`, `_prepare_challenger_row`, `challenger_reason_codes`)
- `tests/test_reasons.py` (NEW — 12 tests)
- `docs/implementation-artifacts/reason-codes-report-2-2.md` (NEW)
